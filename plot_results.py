...
def run_single_trace_sft(trace: Mapping[str, object], sft_model) -> Dict[str, List[float]]:  # noqa: ANN001
    sim = AutoscaleSimulator(
        config=AutoscaleSimConfig(episode_length=len(trace["rps"]), history_length=len(trace["rps"])),
        trace=trace["rps"],
        seed=None,
...
def make_aggregate_bar_chart(
    ...
    metrics = ["avg_reward", "mean_latency_ms", "avg_ready_pods", "drop_fraction"]
    ...
    bars_h = ax.bar([i - width / 2 for i in x], heuristic_vals, width=width, label="Heuristic")
    bars_s = ax.bar([i + width / 2 for i in x], sft_vals, width=width, label="SFT")
    ax.set_xticks(x)
...
def make_aggregate_metric_subplots(
    ...
    for i, metric in enumerate(metrics):
        ax = flat_axes[i]
        values = [float(heuristic_agg[metric]), float(sft_agg[metric])]
        bars = ax.bar(["Heuristic", "SFT"], values, width=0.55)
        ax.set_title(metric)
        ax.grid(axis="y", alpha=0.25)
...
def main() -> None:
    ...
    if sft_model is None:
        raise RuntimeError(
            f"Could not load SFT model from {args.sft_model_path!r}. "
            "Train model first or provide a valid path."
        )
    ...
    sft_series = run_single_trace_sft(single_trace, sft_model)
    single_path = args.output_dir / "single_trace_heuristic_vs_sft.png"
    ...
    agg_path = args.output_dir / "aggregate_heuristic_vs_sft.png"
    make_aggregate_bar_chart(
        output_path=agg_path,
        heuristic_agg=heuristic_agg,
        sft_agg=sft_agg,
    )
    agg_subplots_path = args.output_dir / "aggregate_heuristic_vs_sft_subplots.png"
    make_aggregate_metric_subplots(
        output_path=agg_subplots_path,
        heuristic_agg=heuristic_agg,
        sft_agg=sft_agg,
    )
    summary_path = args.output_dir / "aggregate_heuristic_vs_sft_summary.json"
